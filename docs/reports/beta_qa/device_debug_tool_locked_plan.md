...
- **Connectivity**: Interactive serial selection if >1 device.
...
3. **Menu Sanity**: Confirm existing video workflow is untouched.
4. **End-to-End**: Run report on physical device (if available) and verify file generation in `debug_reports/`.
