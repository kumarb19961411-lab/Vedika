...
export const createBooking = functions.https.onCall(async (data, context) => {
  ...
  if (!vendorId || !eventDate || !eventType || !slotType || !idempotencyKey) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Missing required parameters: vendorId, eventDate, eventType, slotType, or idempotencyKey.'
    );
  }
  ...
  const customerName = userDoc.exists ? userDoc.data()?.name || 'Unknown' : 'Unknown';
  ...
  try {
    return await db.runTransaction(async (transaction) => {
      ...
      const currentSlots: string[] = occupancyData?.slotsOccupied || [];
      const currentJobCount: number = occupancyData?.jobCount || 0;
