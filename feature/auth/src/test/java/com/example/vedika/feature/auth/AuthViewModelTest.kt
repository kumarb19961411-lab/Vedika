...
class AuthViewModelTest {
    ...
    fun `verifyOtp as Partner with existing profile updates to Verified with profileExists true`() = runTest {
        val uid = "uid_123"
        val otp = "123456"
        val user = AppUser(uid, "Owner Name", "9876543210")
        
        coEvery { authRepository.verifyOtp(any(), otp) } returns Result.success(user)
        coEvery { vendorRepository.getVendorProfile(uid) } returns Result.success(
            VendorProfile(id = uid, businessName = "Business", ownerName = "Owner", vendorType = VendorType.VENUE)
        )
    ...
    fun `loginAsDevBypass follows canonical role resolution path`() = runTest {
        val uid = "dev_uid"
        val user = AppUser(uid, "Dev User", "0000000000")
        
        coEvery { authRepository.loginAsDevBypass("dev") } returns Result.success(user)
        coEvery { vendorRepository.getVendorProfile(uid) } returns Result.success(
            VendorProfile(id = uid, businessName = "Dev Business", ownerName = "Dev Owner", vendorType = VendorType.VENUE)
        )
        every { sessionStorage.saveAccountMode(AccountMode.PARTNER) } returns Unit
